...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  6 23:17:50 2023
...
p_flow_flux=np.floor(file_flux['P_FLOW_FLUX_MONTH_1'][inx_t])
t1=datetime(2016, 1, 1)
t2=datetime(2025, 12, 31)
t1=t1.toordinal()+366
t2=t2.toordinal()+366
inx_t=np.where((time1>=t1) & (time1<=t2))[0]
p_sea_flux=np.floor(file_flux['P_SEA_FLUX_MONTH_1'][inx_t])
...
p_flow_flux=p_flow_flux.reshape(1,-1)
p_sea_flux=p_sea_flux.reshape(1,-1)
...
depth=depth[inx_x]
time=time[inx_x]
p_flow_flux=p_flow_flux[inx_x]
p_sea_flux=p_sea_flux[inx_x]
...
p_flow_flux=p_flow_flux[:,np.newaxis]
p_sea_flux=p_sea_flux[:,np.newaxis]
...
if normaliztation==3:
    ...
    X=(X-X_mean)/X_std
    X[:,4]=flux_log
sio.savemat('X_P.mat',{'X':X})
X=torch.from_numpy(X).float()

###Post-normalization perturbation`

all_set = DD.TensorDataset(X)
batch=1000000
gpu0_bsz=8
acc_grad=1
train_loader = torch.utils.data.DataLoader(all_set, batch_size=batch,pin_memory=False)
hat_q=[]
tt=[]

net, params = torch.load('mlp_model_159159.pth', map_location=device)
...
sio.savemat('Y_P.mat', {'Y':quality_hat})
if normaliztation==4:
...
sio.savemat(('mlp_fvcom_pred_'+'1521-4.5-2019-2025'+'_0507(16-21).mat'), {'p_hat':p_hat,'lat':lat,'lon':lon,'depth':depth,'sal':sal,'time':time})
