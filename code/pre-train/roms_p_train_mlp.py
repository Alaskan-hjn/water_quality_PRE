...
# -*- coding: utf-8 -*-
"""
Created on Mon May 29 10:50:13 2023
...
from data_parallel_my_v2 import *
...
# CHOOSE FLUX DATA
p_flow_flux=np.floor(file_flux['P_FLOW_FLUX_7DAYS_1'][inx_t])
p_sea_flux=np.floor(file_flux['P_SEA_FLUX_7DAYS_1'][inx_t])
# CHOOSE ROMS DATA
time_1=file['TIME_1'][:]
...
n=n[inx]
time=time[inx]
p_flow_flux=p_flow_flux[inx]
p_sea_flux=p_sea_flux[inx]
...
if continue_training == False:
    if torch.cuda.device_count()>3:
        ...
        net.to(device)
        # net.to(device)  # 这句不能少，最好不要用model.cuda()
        # net = torch.nn.parallel.DistributedDataParallel(net, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=True)  # 这句加载到多GPU上
    else:
        net=MLP(neuron_number).to(device)
    mm=[]
    tt=[]
else:
    net,params=torch.load(('mlp_model_155.pth'))
    mm=params['train_RMSE']
    tt=params['test_RMSE']
loss0 = nn.MSELoss(reduction='mean').to(device)
optimizer = torch.optim.Adam(net.parameters(), lr=params['learning_rate'], weight_decay=params['decay'])
